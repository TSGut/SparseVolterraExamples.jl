...
As an unregistered Julia package, you can install this package via ```] add https://github.com/TSGut/SparseVolterraExamples.jl``` or alternatively ```Pkg.add("https://github.com/TSGut/SparseVolterraExamples.jl")```. Test whether everything works as intended by executing ```Pkg.test("SparseVolterraExamples")```. Assuming this executes without errors, you can then explore the files in the examples folders.
